...
class FirefoxOptionsProvider extends OptionsProvider<FirefoxOptions> {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(FirefoxOptionsProvider.class);
  ...
  private void logBinary(FirefoxOptions options) {
    ...
    LOG.debug("Firefox binary: " + binary);
  }
    ...
    if (LOG.isTraceEnabled()) {
      ...
      if (capability instanceof LoggingPreferences) {
        ...
        for (String logType : enabledLogTypes) {
          ...
          LOG.trace("Firefox logging enabled: " + logType + " -> " + level);
        }
      }
    }
  }
  ...
  private void logProfile(FirefoxOptions options) {
    ...
    if (profile != null) {
      try {
        LOG.trace("Firefox profile: " + profile.toJson());
      }
      catch (IOException ex) {
        LOG.debug("Exception when dumping Firefox profile to JSON.", ex);
      }
    }
  }
  ...
  private void setBinary(FirefoxOptions options) {
    ...
    if (StringUtils.isNotBlank(firefoxBin)) {
      LOG.debug("webdriver.firefox.bin: '" + firefoxBin + "'");
      File binaryFile = new File(firefoxBin);
      if (binaryFile.isFile()) {
        LOG.trace("Setting explicit binary for Firefox: '" + binaryFile.getPath() + "'");
        FirefoxBinary binary = new FirefoxBinary(binaryFile);
        binary.addCommandLineOptions("-log", "fatal");
        options.setBinary(binary);
      }
      else {
        LOG.warn("Skipping explicit binary for Firefox because it is not a file: '" + binaryFile.getPath() + "'");
      }
    }
  }
  ...
  protected FirefoxOptions getBrowserSpecificOptions() {
    LOG.debug("creating capabilities for Firefox");
  ...
  protected void log(FirefoxOptions options) {
    LOG.debug("Firefox options: " + options);
    logProfile(options);
    logBinary(options);

...
public class SelectorPojo implements NestedSelector {
  ...
  public SelectorPojo(SpecPojo spec, NestedSelector selector) {
    if (spec == null || selector == null) {
      throw new GaleniumException("cannot construct pojo from: " + spec + " and " + selector);
    }
    setSpec(spec);
  ...
  public NestedSelector getParent() {
    NestedSelector parent = getDelegatee().getParent();
    if (parent != null) {
      return wrap(parent);
    }
    return null;
   ...
   *
   * @return a  {@link io.wcm.qa.glnm.maven.specs.pojo.SpecPojo} object.
   */

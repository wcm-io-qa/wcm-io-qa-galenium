...
public class Navigation extends AbstractWebDriverPageObject {
  ...
  public List<NavigationTopLevelEntry> getTopLevelItems() {
    if (navItems == null) {
      navItems = new ArrayList<NavigationTopLevelEntry>();
      openNavigation();
      List<WebElement> elements = getDriver().findElements(SELECTOR_TOP_LEVEL_ITEMS.asBy());
      for (WebElement webElement : elements) {
        navItems.add(new NavigationTopLevelEntry(webElement));
      }
    }
  ...
  private void openNavigation() {
    // will always be open on desktop
    if (isMobile()) {
